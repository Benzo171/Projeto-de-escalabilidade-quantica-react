...
const RoteamentoAereoTailwind = () => {
    ...
    useEffect(() => { localStorage.setItem('numAirports', numAirports); }, [numAirports]);
    useEffect(() => { localStorage.setItem('numRMRCities', numRMRCities); }, [numRMRCities]);
    ...
    // Funções de visualização (abreviadas, mantendo lógica original)
    const updateAirportVisualization = useCallback((count, ref) => {
        ...
        if (count > 1) {
            ...
            for (let i = 0; i < airports.length - 1; i++) {
                ...
                line.setAttribute('x1', airports[i].x); line.setAttribute('y1', airports[i].y);
                line.setAttribute('x2', airports[i + 1].x); line.setAttribute('y2', airports[i + 1].y);
                line.setAttribute('stroke', '#A855F7'); line.setAttribute('stroke-width', '1'); line.setAttribute('stroke-dasharray', '5,5'); line.setAttribute('opacity', '0.5');
                svg.appendChild(line);
            }
            const line = document.createElementNS('http://www.w3.org/2000/svg', 'line');
            line.setAttribute('x1', airports[airports.length - 1].x); line.setAttribute('y1', airports[airports.length - 1].y);
            line.setAttribute('x2', airports[0].x); line.setAttribute('y2', airports[0].y);
            line.setAttribute('stroke', '#A855F7'); line.setAttribute('stroke-width', '1'); line.setAttribute('stroke-dasharray', '5,5'); line.setAttribute('opacity', '0.5');
            svg.appendChild(line);
            div.appendChild(svg);
        }
    ...
    const updateRMRVisualization = useCallback((count, ref) => {
        ...
        if (!div) return;
        div.innerHTML = ''; 
        if (count < 2) { div.textContent = count === 0 ? 'Mínimo de 2 cidades necessário.' : 'Mínimo de 2 cidades necessário.'; return; }
        const cidadesAtuais = CIDADES_TOTAIS.slice(0, count);
        const width = div.offsetWidth; const height = div.offsetHeight; const maxCoord = 900; const scaleX = width / (maxCoord * 1.2); const scaleY = height / (maxCoord * 1.2); 
        const airports = cidadesAtuais.map((cidade, index) => {
            const x = cidade.x * scaleX + (width * 0.1); const y = cidade.y * scaleY + (height * 0.1);
        ...
        airports.forEach(airport => {
            ...
            cityDot.style.left = `${airport.x}px`; cityDot.style.top = `${airport.y}px`; cityDot.style.transform = 'translate(-50%, -50%)'; cityDot.title = `Cidade: ${airport.nome}`;
            div.appendChild(cityDot);
            const cityName = document.createElement('span');
            cityName.className = 'absolute text-xs text-gray-200 pointer-events-none whitespace-nowrap';
            cityName.style.left = `${airport.x + 8}px`; cityName.style.top = `${airport.y}px`; cityName.style.transform = 'translateY(-50%)'; cityName.textContent = airport.nome;
    ...
    useEffect(() => { updateAirportVisualization(numAirports, airportVisualizationRef); }, [numAirports, updateAirportVisualization]);
    useEffect(() => { updateRMRVisualization(numRMRCities, airportVisualizationRMRRef); }, [numRMRCities, updateRMRVisualization]);

    const { classicalSteps, quantumSteps } = simulateComputationSteps(numAirports);
    const classicalColor = '#A855F7'; const quantumColor = '#7C3AED'; const rmrFatorialColor = '#C4A1FF'; const rmrQuantumColor = '#7C3AED'; 

    const chartData = {
        labels: ['Clássico', 'Quântico'],
        datasets: [{
            label: 'Passos (Operações)', data: [classicalSteps, quantumSteps], backgroundColor: [classicalColor, quantumColor], borderColor: [classicalColor, quantumColor], borderWidth: 1, borderRadius: 8,
        }]
    };
    const chartOptions = {
        responsive: true, maintainAspectRatio: false,
        scales: { y: { beginAtZero: true, title: { display: true, text: 'Passos Computacionais ', color: '#9ca3af' }, ticks: { color: '#9ca3af', callback: (value) => formatSteps(value) }, type: 'logarithmic', grid: { color: 'rgba(255, 255, 255, 0.1)' } }, x: { ticks: { color: '#9ca3af', font: { size: 14 }, maxRotation: 0, minRotation: 0, }, grid: { color: 'rgba(255, 255, 255, 0.1)' } } },
        plugins: { legend: { display: false }, tooltip: { callbacks: { label: function(context) { let label = context.dataset.label || ''; if (label) { label += ': '; } if (context.parsed.y !== null) { label += formatSteps(context.parsed.y); label += ` (${context.parsed.y.toExponential(2)})`; } return label; } } } }
    };
    
    const comparisonDataPoints = Array.from({ length: numAirports }, (_, i) => { const N = i + 1; const { classicalSteps: c, quantumSteps: q } = simulateComputationSteps(N); return { N, classical: c, quantum: q }; });
    const ratioChartData = {
        labels: comparisonDataPoints.map(p => p.N),
        datasets: [ { label: 'Clássico (O(2^N))', data: comparisonDataPoints.map(p => p.classical), backgroundColor: classicalColor, borderColor: classicalColor, borderWidth: 3, fill: false, tension: 0.1, pointRadius: 4, pointBackgroundColor: classicalColor, pointBorderColor: '#fff', }, { label: 'Quântico (O(N^2))', data: comparisonDataPoints.map(p => p.quantum), backgroundColor: quantumColor, borderColor: quantumColor, borderWidth: 3, fill: false, tension: 0.1, pointRadius: 4, pointBackgroundColor: quantumColor, pointBorderColor: '#fff', } ],
    };
    const ratioChartOptions = {
        responsive: true, maintainAspectRatio: false,
        scales: { y: { title: { display: true, text: 'Passos Computacionais ', color: '#9ca3af' }, ticks: { color: '#9ca3af', callback: (value) => formatSteps(value) }, type: 'linear', grid: { color: 'rgba(255, 255, 255, 0.1)' }, beginAtZero: true, }, x: { title: { display: true, text: 'Número de Aeroportos (N)', color: '#9ca3af' }, ticks: { color: '#9ca3af' }, grid: { color: 'rgba(255, 255, 255, 0.1)' } } },
        plugins: { legend: { display: true, position: 'top', labels: { color: '#fff' } }, tooltip: { callbacks: { title: (context) => `N = ${context[0].label}`, label: (context) => { const datasetLabel = context.dataset.label || ''; const value = context.parsed.y; return `${datasetLabel}: ${formatSteps(value)} passos`; } } } }
    };
    ...
    const rmrBarChartData = {
        labels: ['Clássico', 'Quântico'],
        datasets: [{ label: 'Passos Computacionais', data: [rmrClassicalSteps, rmrQuantumSteps], backgroundColor: [rmrFatorialColor, rmrQuantumColor], borderColor: [rmrFatorialColor, rmrQuantumColor], borderWidth: 1, borderRadius: 8, }]
    };
    const rmrBarChartOptions = {
        responsive: true, maintainAspectRatio: false,
        scales: { y: { beginAtZero: true, title: { display: true, text: 'Passos Computacionais ', color: '#9ca3af' }, ticks: { color: '#9ca3af', callback: function(value) { return formatSteps(value); } }, type: 'logarithmic', grid: { color: 'rgba(255, 255, 255, 0.1)' } }, x: { ticks: { color: '#9ca3af', font: { size: 14 }, maxRotation: 0, minRotation: 0, }, grid: { color: 'rgba(255, 255, 255, 0.1)' } } },
        plugins: { legend: { display: false }, tooltip: { callbacks: { label: function(context) { let label = context.dataset.label || ''; if (label) { label += ': '; } if (context.parsed.y !== null) { label += formatSteps(context.parsed.y); label += ` (${context.parsed.y.toExponential(2)})`; } return label; } } } }
    };

    const rmrDataPoints = Array.from({ length: numRMRCities }, (_, i) => { const N = i + 1; const routes = calculateTSPRoutes(N) / 1000; const quantumTime = simulateQuantumTSPSpeedup(N); return { N, routes, quantumTime }; });
    const rmrRoutesChartData = {
        labels: rmrDataPoints.map(p => p.N),
        datasets: [ { label: 'Clássico: Rotas Possíveis (O(N!))', data: rmrDataPoints.map(p => p.routes), backgroundColor: rmrFatorialColor, borderColor: rmrFatorialColor, borderWidth: 3, fill: false, tension: 0.1, pointRadius: 4, pointBackgroundColor: rmrFatorialColor, pointBorderColor: '#fff', pointHitRadius: 10, }, { label: 'Quântico: Rotas possíveis. (O(N^3))', data: rmrDataPoints.map(p => p.quantumTime), backgroundColor: rmrQuantumColor, borderColor: rmrQuantumColor, borderWidth: 3, fill: false, tension: 0.1, pointRadius: 4, pointBackgroundColor: rmrQuantumColor, pointBorderColor: '#fff', pointHitRadius: 10, }, ],
    };
    const rmrRoutesChartOptions = {
        responsive: true, maintainAspectRatio: false,
        scales: { y: { title: { display: true, text: 'Número de Operações/Rotas ', color: '#9ca3af' }, ticks: { color: '#9ca3af', callback: (value) => formatSteps(value) }, type: 'logarithmic', min: 1, grid: { color: 'rgba(255, 255, 255, 0.1)' }, }, x: { title: { display: true, text: 'Número de Cidades (N)', color: '#9ca3af' }, ticks: { color: '#9ca3af' }, grid: { color: 'rgba(255, 255, 255, 0.1)' } } },
        plugins: { legend: { display: true, position: 'top', labels: { color: '#fff' } }, tooltip: { callbacks: { title: (context) => `Cidades (N) = ${context[0].label}`, label: (context) => { const value = context.parsed.y; const label = context.dataset.label; const formattedValue = value.toLocaleString('pt-BR', { maximumFractionDigits: 0 }); return `${label}: ${formattedValue} (${value.toExponential(2)})`; } } } }
    };

    const handleSliderChange = (event) => { setNumAirports(parseInt(event.target.value)); };
    const handleRMRSliderChange = (event) => { setNumRMRCities(parseInt(event.target.value)); };
...
// ...
                            ...
                            <div className="flex-1 p-6 flex flex-col justify-between min-w-[300px]">
                                <div className="mb-8">
                                    ...
                                    <input type="range" id="num-airports-sim" min="0" max="25" value={numAirports} onChange={handleSliderChange} className="w-full h-2 bg-indigo-800 rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:bg-purple-500 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:shadow-lg" />
                                    <div className="flex justify-between text-xs text-gray-400 mt-1"> <span>0</span> <span>25</span> </div>
                                </div>
                                <div className="flex-grow flex flex-col justify-center">
                                    ...
                                    <div className="w-full h-auto max-h-[220px]"> <Bar data={chartData} options={chartOptions} /> </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="w-full max-w-[1000px] mx-auto mb-8 font-['Inter',_sans-serif]">
                        <div className="bg-gray-950/80 backdrop-blur-md rounded-xl shadow-2xl overflow-hidden border border-purple-700 p-6">
                            <h3 className="text-gray-100 mb-5 text-xl font-semibold text-center"> Curvas de Complexidade Algorítmica (Escala Linear) 📊 </h3>
                            <div className="w-full h-[250px] flex justify-center items-center"> <Line data={ratioChartData} options={ratioChartOptions} /> </div>
                            <p className="text-sm text-gray-400 text-center mt-4"> A diferença de complexidade entre o algoritmo Clássico (Exponencial) e o Quântico (Polinomial) torna-se dramaticamente evidente conforme o problema cresce. Na escala linear, a curva Clássica domina rapidamente o gráfico. </p>
                        </div>
                    </div>
                    ...
                    <div className="w-full max-w-[1000px] mx-auto my-8 font-['Inter',_sans-serif]">
                         <div className="flex flex-col lg:flex-row bg-gray-950/80 backdrop-blur-md rounded-xl shadow-2xl overflow-hidden border border-purple-600/80">
                            <div className="flex-2 p-6 border-b lg:border-b-0 lg:border-r border-purple-600/80 flex flex-col items-center justify-center min-h-[350px]">
                                <h3 className="text-gray-100 mb-5 text-xl font-semibold text-center"> Exemplo Prático: Roteamento entre cidades: ({numRMRCities} Cidades) 📍 </h3>
                                <div className="mb-6 w-full max-w-[300px] mx-auto">
                                    <label htmlFor="num-rmr-cities" className="block mb-2 font-bold text-sm text-gray-100 text-center"> Cidades no Roteiro (N): <span className="text-purple-400 font-extrabold">{numRMRCities}</span> </label>
...
>
    {numRMRCities < 2 ? 'Selecione no mínimo 2 cidades.' : null}
                            ...
                            <div className="flex-1 p-6 flex flex-col justify-between min-w-[300px]">
                                ...
                                <div className="w-full h-auto max-h-[340px] flex-grow"> <Bar data={rmrBarChartData} options={rmrBarChartOptions} /> </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="w-full max-w-[1000px] mx-auto mb-8 font-['Inter',_sans-serif]">
                        <div className="bg-gray-950/80 backdrop-blur-md rounded-xl shadow-2xl overflow-hidden border border-purple-700/80 p-6">
                            <h3 className="text-gray-100 mb-5 text-xl font-semibold text-center"> Comparação de Complexidade no Roteamento (Escala Logarítmica) 🚀 </h3>
                            <div className="w-full h-[250px] flex justify-center items-center"> <Line data={rmrRoutesChartData} options={rmrRoutesChartOptions} /> </div>
                            <p className="text-sm text-gray-400 text-center mt-4"> O gráfico logarítmico revela a diferença brutal entre a curva, que se torna quase vertical, e a curva simulada. O ganho de eficiência no algoritmo quântico é a chave para resolver problemas como o TSP em escala industrial. </p>
                        </div>
                    </div>

                    {/* --- NAVEGAÇÃO ENTRE OS EXEMPLOS (NOVA GRADE) --- */}
                    <div className="w-full max-w-[1000px] mx-auto mt-20 mb-12">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div className="p-5 rounded-xl border border-purple-500 bg-purple-900/30 cursor-default">
                                <h4 className="font-bold text-lg mb-2 text-white">✈️ Roteamento (TSP)</h4>
                    ...
                    {/* --- BOTÃO GRANDE DE VOLTAR --- */}
                    <div className="w-full max-w-[1000px] mx-auto mt-12 mb-20 flex justify-center">
                        <button
                            onClick={handleBackToCategories}
                            className="flex items-center justify-center gap-3 py-4 px-10 bg-purple-900/80 border border-purple-500/50 rounded-xl text-white font-bold hover:bg-purple-800 hover:scale-105 transition-all duration-300 shadow-lg cursor-pointer z-50 relative"
                        >
            ...
            {/* Espaço extra no final para garantir que nada corte o botão */}
