...
import portfolioIcon from '../assets/assets/imgs_icones/OtimizacaoPortfolio.png';
...
const FundamentosPage = () => {
  ...
  return (
    <div className="min-h-screen text-white py-20" style={{ backgroundColor: '#111' }}>
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {cards.map((card) => (
            <div 
              key={card.id}
              // Cor de fundo roxa escura e arredondamento
              className="formula-card bg-[#5C2D91] rounded-xl p-6 flex flex-col justify-between shadow-2xl transition-all duration-300 hover:scale-[1.02] hover:shadow-purple-500/50"
            >
              <div className="flex flex-col items-center text-center h-full">
                ...
                {/* Descrição */}
                <p className="text-gray-200 text-base leading-relaxed mt-4 mb-6 flex-grow">
                  {card.description}
                </p>
              </div>
              
              {/* Botão "Ver Fórmula" - Cor roxa mais escura e efeito hover */}
              <a 
                href={card.link}
                // Cor de fundo do botão: #4A1C70 (um roxo mais escuro)
                className="ver-formula-btn bg-[#4A1C70] border-none py-3 px-6 text-white text-lg font-semibold text-center rounded-lg cursor-pointer mt-4 transition-all duration-300 hover:bg-white hover:text-[#4A1C70] inline-block text-decoration-none shadow-lg"
              >
                Ver Fórmula
              </a>
            </div>
      ...
      {/* Explore Section - Removido para simplificar e focar no layout principal */}
